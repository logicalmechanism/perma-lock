...
perma_lock_min_utxo=$(${cli} transaction calculate-min-required-utxo \
    --babbage-era \
    --protocol-params-file ./tmp/protocol.json \
    --tx-out-reference-script-file ${perma_lock_script_path} \
    --tx-out="${script_reference_address} + 1000000" | tr -dc '0-9')

perma_lock_script_reference_utxo="${script_reference_address} + ${perma_lock_min_utxo}"
...
echo -e "\nCreating Perma Lock Script:" ${perma_lock_script_reference_utxo}
...
${cli} transaction build-raw \
    ...
    --tx-out="${perma_lock_script_reference_utxo}" \
    --tx-out-reference-script-file ${perma_lock_script_path} \
    --fee 0
...
#
firstReturn=$((${starting_reference_lovelace} - ${perma_lock_min_utxo} - ${fee}))
